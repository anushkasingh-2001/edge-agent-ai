 ...
 *
 * Mode awareness adds, on top of the original bulk flow:
 ...
 *
 * The existing `confirmSuppressions` / `persistSuppressions` helper
 * (for "needs_user_decision" clusters a reviewer accepts as FP) is
 * preserved — it is a manual, audited write to `.edgeagent/
 * suppressions.json` and never auto-runs.
 */
...
interface BulkBody {
  ...
  /** Intelligence mode (defaults to "auto"). Save returns templates-only. */
...
export async function POST(req: Request) {
  ...
  // ---- Pre-flight COST ESTIMATE + budget gate (mode-aware) ----
  // One LLM call per cluster — never per finding.
  ...
  return NextResponse.json({
    ...
    total_findings: planned.length,
    cluster_count: clusters.length,
    estimated_llm_calls: policy.allowPatchGeneration ? llmCallEstimate : 0,
    estimate,
    budget,
    clusters: ordered,
  })
