/**
 * Patch Confidence (NEW FILE → lib/patch-confidence.ts)
 *
 * Turns the objective validation signals into a single score + a UI band.
 * The re-scan remains the binary ARBITER (finding gone or not); this layer
 * exists purely for user trust — it tells the user how much to scrutinize
 * before clicking Apply.
 *
 * HARD RULE: every input here is an objective, machine-checked signal.
 * The LLM never rates its own patch. If we ever add an "LLM self-confidence"
 * field, it must NOT feed this score.
 */

export interface ValidationSignals {
  /** Re-scan after applying the patch: did the original finding disappear? */
  findingResolved: boolean
  /** Patched file still parses (LibCST / TS parser). */
  parses: boolean
  ...
  /** Tests ran and passed. null = not run (no test target found / skipped). */
 ...
 * GATES: if either fails, the patch can never be "strong", regardless of
 * other signals.
 */
export function scorePatch(s: ValidationSignals): ConfidenceResult {
  ...
  // Hard gates.
  const gatedFail = !s.findingResolved || !s.parses || !s.diffApplied || !s.touchedAllowedFilesOnly
  ...
  score += s.testsPassed === true ? 10 : 0 // null → 0, but not penalized below
  score += s.buildPassed === true ? 5 : 0
  score += s.diffLines <= 15 ? 5 : s.diffLines <= 40 ? 2 : 0
  score += s.matchesStyle ? 5 : 0

  // If tests/build weren't run, cap the ceiling so we never show "strong"
  // for an unverified-by-tests patch on a repo that HAS tests.
